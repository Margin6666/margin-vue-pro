...
依赖：docker compose v2，删除`name: margin-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name margin-maven-repo

docker run -it --rm --name margin-maven \
    -v margin-maven-repo:/root/.m2 \
